...
async function renderGraph(graph: HTMLElement, fullSlug: FullSlug) {
  ...
  function nodeRadius(d: NodeData) {
    ...
    return (2 + Math.pow(numLinks, 0.7) * 4) * (nodeSizeMultiplier ?? 1)
  }
  ...
  if (enableZoom) {
    ...
    const selection = select<HTMLCanvasElement, NodeData>(app.canvas).call(zoomBehavior)
    if (scale !== 1) {
      selection.call(zoomBehavior.scaleTo, scale)
    }
  }
